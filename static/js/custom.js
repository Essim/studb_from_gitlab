if (typeof jQuery === 'undefined') throw new Error('AdminLTE requires jQuery'); +(function (a) {
  function b(b) {
    return this.each(function () {
      let e = a(this),
        g = e.data(c); if (!g) { const h = a.extend({}, d, e.data(), typeof b === 'object' && b); e.data(c, g = new f(e, h)); } if (typeof g === 'string') { if (void 0 === g[b]) throw new Error(`No method named ${b}`); g[b](); }
    });
    ...
    f = function (b, c) { if (this.element = b, this.options = c, this.$overlay = a(c.overlay), c.source === '') throw new Error('Source url was not defined. Please specify a url in your BoxRefresh source option.'); this._setUpListeners(), this.load(); }; f.prototype.load = function () { this._addOverlay(), this.options.onLoadStart.call(a(this)), a.get(this.options.source, this.options.params, (b) => { this.options.loadInContent && a(this.options.content).html(b), this.options.onLoadDone.call(a(this), b), this._removeOverlay(); }, this.options.responseType !== '' && this.options.responseType); }, f.prototype._setUpListeners = function () { a(this.element).on('click', e.trigger, (a) => { a && a.preventDefault(), this.load(); }); }, f.prototype._addOverlay = function () { a(this.element).append(this.$overlay); }, f.prototype._removeOverlay = function () { a(this.element).remove(this.$overlay); }; const g = a.fn.boxRefresh; a.fn.boxRefresh = b, a.fn.boxRefresh.Constructor = f, a.fn.boxRefresh.noConflict = function () { return a.fn.boxRefresh = g, this; }, a(window).on('load', () => { a(e.data).each(function () { b.call(a(this)); }); });
}(jQuery)), (function (a) {
  function b(b) {
    return this.each(function () {
      let e = a(this),
        f = e.data(c); if (!f) { const g = a.extend({}, d, e.data(), typeof b === 'object' && b); e.data(c, f = new h(e, g)); } if (typeof b === 'string') { if (void 0 === f[b]) throw new Error(`No method named ${b}`); f[b](); }
    });
  } var c = 'lte.boxwidget',
    d = {
    ...
    e = {
      data: '.box', collapsed: '.collapsed-box', body: '.box-body', footer: '.box-footer', tools: '.box-tools',
    },
    f = { collapsed: 'collapsed-box' },
    g = { collapsed: 'collapsed.boxwidget', expanded: 'expanded.boxwidget', removed: 'removed.boxwidget' },
    h = function (a, b) { this.element = a, this.options = b, this._setUpListeners(); }; h.prototype.toggle = function () { a(this.element).is(e.collapsed) ? this.expand() : this.collapse(); }, h.prototype.expand = function () {
    let b = a.Event(g.expanded),
      c = this.options.collapseIcon,
      d = this.options.expandIcon; a(this.element).removeClass(f.collapsed), a(this.element).find(e.tools).find(`.${d}`).removeClass(d)
      .addClass(c), a(this.element).find(`${e.body}, ${e.footer}`).slideDown(this.options.animationSpeed, () => { a(this.element).trigger(b); });
  }, h.prototype.collapse = function () {
    let b = a.Event(g.collapsed),
      c = this.options.collapseIcon,
      d = this.options.expandIcon; a(this.element).find(e.tools).find(`.${c}`).removeClass(c)
      .addClass(d), a(this.element).find(`${e.body}, ${e.footer}`).slideUp(this.options.animationSpeed, () => { a(this.element).addClass(f.collapsed), a(this.element).trigger(b); });
  }, h.prototype.remove = function () { const b = a.Event(g.removed); a(this.element).slideUp(this.options.animationSpeed, () => { a(this.element).trigger(b), a(this.element).remove(); }); }, h.prototype._setUpListeners = function () { const b = this; a(this.element).on('click', this.options.collapseTrigger, (a) => { a && a.preventDefault(), b.toggle(); }), a(this.element).on('click', this.options.removeTrigger, (a) => { a && a.preventDefault(), b.remove(); }); }; const i = a.fn.boxWidget; a.fn.boxWidget = b, a.fn.boxWidget.Constructor = h, a.fn.boxWidget.noConflict = function () { return a.fn.boxWidget = i, this; }, a(window).on('load', () => { a(e.data).each(function () { b.call(a(this)); }); });
}(jQuery)), (function (a) {
  function b(b) {
    return this.each(function () {
      let e = a(this),
        f = e.data(c); if (!f) { const g = a.extend({}, d, e.data(), typeof b === 'object' && b); e.data(c, f = new h(e, g)); } typeof b === 'string' && f.toggle();
    });
  ...
  function b(b) {
    return this.each(function () {
      let d = a(this),
        e = d.data(c); e || d.data(c, e = new f(d)), typeof b === 'string' && e.toggle(d);
    });
  } var c = 'lte.directchat',
    d = { data: '[data-widget="chat-pane-toggle"]', box: '.direct-chat' },
    e = { open: 'direct-chat-contacts-open' },
    f = function (a) { this.element = a; }; f.prototype.toggle = function (a) { a.parents(d.box).first().toggleClass(e.open); }; const g = a.fn.directChat; a.fn.directChat = b, a.fn.directChat.Constructor = f, a.fn.directChat.noConflict = function () { return a.fn.directChat = g, this; }, a(document).on('click', d.data, function (c) { c && c.preventDefault(), b.call(a(this), 'toggle'); });
}(jQuery)), (function (a) {
  function b(b) {
    return this.each(function () {
      let e = a(this),
        f = e.data(c); if (!f) { const h = a.extend({}, d, e.data(), typeof b === 'object' && b); e.data(c, f = new g(h)); } if (typeof b === 'string') { if (void 0 === f[b]) throw new Error(`No method named ${b}`); f[b](); }
    });
  } var c = 'lte.layout',
    ...
    g = function (a) { this.options = a, this.bindedResize = !1, this.activate(); }; g.prototype.activate = function () { this.fix(), this.fixSidebar(), a('body').removeClass(f.holdTransition), this.options.resetHeight && a(`body, html, ${e.wrapper}`).css({ height: 'auto', 'min-height': '100%' }), this.bindedResize || (a(window).resize(() => { this.fix(), this.fixSidebar(), a(`${e.logo}, ${e.sidebar}`).one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend', () => { this.fix(), this.fixSidebar(); }); }), this.bindedResize = !0), a(e.sidebarMenu).on('expanded.tree', () => { this.fix(), this.fixSidebar(); }), a(e.sidebarMenu).on('collapsed.tree', () => { this.fix(), this.fixSidebar(); }); }, g.prototype.fix = function () {
    a(`${e.layoutBoxed} > ${e.wrapper}`).css('overflow', 'hidden'); let b = a(e.mainFooter).outerHeight() || 0,
      c = a(e.mainHeader).outerHeight() + b,
      d = a(window).height(),
      g = a(e.sidebar).height() || 0; if (a('body').hasClass(f.fixed))a(e.contentWrapper).css('min-height', d - b); else { let h; d >= g ? (a(e.contentWrapper).css('min-height', d - c), h = d - c) : (a(e.contentWrapper).css('min-height', g), h = g); const i = a(e.controlSidebar); void 0 !== i && i.height() > h && a(e.contentWrapper).css('min-height', i.height()); }
  }, g.prototype.fixSidebar = function () { if (!a('body').hasClass(f.fixed)) return void (void 0 !== a.fn.slimScroll && a(e.sidebar).slimScroll({ destroy: !0 }).height('auto')); this.options.slimscroll && void 0 !== a.fn.slimScroll && a(e.sidebar).slimScroll({ height: `${a(window).height() - a(e.mainHeader).height()}px`, color: 'rgba(0,0,0,0.2)', size: '3px' }); }; const h = a.fn.layout; a.fn.layout = b, a.fn.layout.Constuctor = g, a.fn.layout.noConflict = function () { return a.fn.layout = h, this; }, a(window).on('load', () => { b.call(a('body')); });
}(jQuery)), (function (a) {
  function b(b) {
    return this.each(function () {
      let e = a(this),
        f = e.data(c); if (!f) { const g = a.extend({}, d, e.data(), typeof b === 'object' && b); e.data(c, f = new h(g)); }b === 'toggle' && f.toggle();
    });
  } var c = 'lte.pushmenu',
  ...
  function b(b) {
    return this.each(function () {
      let e = a(this),
        f = e.data(c); if (!f) { const h = a.extend({}, d, e.data(), typeof b === 'object' && b); e.data(c, f = new g(e, h)); } if (typeof f === 'string') { if (void 0 === f[b]) throw new Error(`No method named ${b}`); f[b](); }
    });
  } var c = 'lte.todolist',
    d = { onCheck(a) { return a; }, onUnCheck(a) { return a; } },
  ...
  function b(b) { return this.each(function () { const e = a(this); if (!e.data(c)) { const f = a.extend({}, d, e.data(), typeof b === 'object' && b); e.data(c, new h(e, f)); } }); } var c = 'lte.tree',
    d = {
    ...
    h = function (b, c) { this.element = b, this.options = c, a(this.element).addClass(f.tree), a(e.treeview + e.active, this.element).addClass(f.open), this._setUpListeners(); }; h.prototype.toggle = function (a, b) {
    let c = a.next(e.treeviewMenu),
      d = a.parent(),
    ...
    const d = a.Event(g.expanded); if (this.options.accordion) {
      let h = c.siblings(e.open),
        i = h.children(e.treeviewMenu); this.collapse(i, h);
    }c.addClass(f.open), b.slideDown(this.options.animationSpeed, () => { a(this.element).trigger(d); });
  }, h.prototype.collapse = function (b, c) { const d = a.Event(g.collapsed); b.find(e.open).removeClass(f.open), c.removeClass(f.open), b.slideUp(this.options.animationSpeed, () => { b.find(`${e.open} > ${e.treeview}`).slideUp(), a(this.element).trigger(d); }); }, h.prototype._setUpListeners = function () { const b = this; a(this.element).on('click', this.options.trigger, function (c) { b.toggle(a(this), c); }); }; const i = a.fn.tree; a.fn.tree = b, a.fn.tree.Constructor = h, a.fn.tree.noConflict = function () { return a.fn.tree = i, this; }, a(window).on('load', () => { a(e.data).each(function () { b.call(a(this)); }); });
}(jQuery));
